...
\hypo{\dots}
\infer1[\textbf{2}]{\Gamma \entails \hoareT{S}{n := llent(t)}{R}}
\infer2[seq]{\Gamma \entails \hoareT{P}{t := [x+1] ; b := llen(t)}{R}}
\hypo{\dots}
\infer1[]{\Gamma \entails \hoareT{R}{n := n + 1}{Q}}
\infer2[seq]{\Gamma \entails \hoareT{P}{t := [x+1] ; b := llen(t); n := n+1}{Q}}
\infer1[exists $\times$ 3]{\Gamma \entails \hoareT{\exists v,\beta,y. P}{t := [x+1] ; b := llen(t); n := n+1}{\exists v,\beta,y. Q}}
\end{prooftree}
\]
...
Then we can see most of the work is just either joining commands naturally via the \textbfr{seq} rule, or unfolding defintitions so we can meet the precondition of a function call. (as can be seen by the $S$ condition).  
